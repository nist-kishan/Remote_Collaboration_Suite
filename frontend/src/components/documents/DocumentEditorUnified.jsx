...
import { useDocument, useDocumentPermissions } from '../../hook/useDocument';
import { useAutoSave } from '../../hook/useAutoSave';
import { useSelector } from 'react-redux';
import { shareDocument } from '../../api/documentApi';
import { useDocumentCollaboration } from '../../hook/useDocumentCollaboration';
import DocumentCollaborators from './DocumentCollaborators';
import DocumentCursors from './DocumentCursors';
                ...
                <Button
                  ...
                  onClick={() => {
                    if (editorRef.current) {
                      const editor = editorRef.current.getEditorElement();
                      if (editor) {
                        const currentZoom = parseFloat(editor.style.transform?.match(/scale\(([^)]+)\)/)?.[1] || '1');
                        editor.style.transform = `scale(${Math.min(currentZoom + 0.1, 2)})`;
                      }
                    }
                  }}
                >
                ...
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    if (editorRef.current) {
                      ...
                      if (editor) {
                        if (zoom === 'Fit to Page') {
                          editor.style.transform = 'scale(0.8)';
                        } else {
                          const zoomValue = parseInt(zoom) / 100;
                          editor.style.transform = `scale(${zoomValue})`;
                        }
                      }
                    }
                  }}
                ...
                <Button
                  ...
                  onClick={() => {
                    if (editorRef.current) {
                      ...
                      if (editor) {
                        const currentZoom = parseFloat(editor.style.transform?.match(/scale\(([^)]+)\)/)?.[1] || '1');
                        editor.style.transform = `scale(${Math.max(currentZoom - 0.1, 0.5)})`;
                      }
                    }
                  }}
                >
